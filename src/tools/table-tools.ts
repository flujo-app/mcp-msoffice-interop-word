...
const addTableSchema = z.object({
  numRows: z
    ...
    .min(1)
    .max(1000)
    .describe("Number of rows for the new table."),
  numCols: z
    .number()
    .int()
    .min(1)
...
async function addTableTool(
  args: z.infer<typeof addTableSchema>,
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully added a ${args.numRows}x${args.numCols} table.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to add table: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const setTableCellTextSchema = z.object({
  tableIndex: z
    ...
    .int()
    .min(1)
    .describe("The 1-based index of the table in the document."),
  rowIndex: z
    .number()
    .int()
    .min(1)
    .describe("The 1-based index of the row within the table."),
  colIndex: z
    .number()
    .int()
    .min(1)
    .describe("The 1-based index of the column within the table."),
  text: z
    .string()
    .max(262144)
    .describe("The text to set in the specified cell."),
});

async function setTableCellTextTool(
  args: z.infer<typeof setTableCellTextSchema>,
): Promise<CallToolResult> {
  try {
    await wordService.setTableCellText(
      args.tableIndex,
      args.rowIndex,
      args.colIndex,
      args.text,
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set text in table ${args.tableIndex}, cell (${args.rowIndex}, ${args.colIndex}).`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to set cell text: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const insertTableRowSchema = z.object({
  tableIndex: z
    ...
    .int()
    .min(1)
    .describe("The 1-based index of the table."),
  beforeRowIndex: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe(
      "Optional: 1-based index of the row to insert before. If omitted, adds row to the end.",
    ),
});

async function insertTableRowTool(
  args: z.infer<typeof insertTableRowSchema>,
): Promise<CallToolResult> {
  try {
    await wordService.insertTableRow(args.tableIndex, args.beforeRowIndex);
    const position = args.beforeRowIndex
      ? `before row ${args.beforeRowIndex}`
      : "at the end";
    return {
      content: [
        {
          type: "text",
          text: `Successfully inserted row into table ${args.tableIndex} ${position}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to insert table row: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const insertTableColumnSchema = z.object({
  ...
  beforeColIndex: z
    ...
    .optional()
    .describe(
      "Optional: 1-based index of the column to insert before. If omitted, adds column to the right end.",
    ),
});

async function insertTableColumnTool(
  args: z.infer<typeof insertTableColumnSchema>,
): Promise<CallToolResult> {
  try {
    await wordService.insertTableColumn(args.tableIndex, args.beforeColIndex);
    const position = args.beforeColIndex
      ? `before column ${args.beforeColIndex}`
      : "at the right end";
    return {
      content: [
        {
          type: "text",
          text: `Successfully inserted column into table ${args.tableIndex} ${position}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to insert table column: ${error.message}`,
        },
      ],
      isError: true,
    };
...
const applyTableAutoFormatSchema = z.object({
  ...
  formatName: z
    ...
    .describe(
      "Name of the table style (e.g., 'Table Grid') or a numeric WdTableFormat enum value.",
    ),
...
async function applyTableAutoFormatTool(
  args: z.infer<typeof applyTableAutoFormatSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully applied format '${args.formatName}' to table ${args.tableIndex}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to apply table format: ${error.message}`,
        },
      ],
      isError: true,
    };
