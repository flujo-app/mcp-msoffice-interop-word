...
const setAlignmentSchema = z.object({
  alignment: z
    ...
    .max(3)
    .describe(
      "Alignment type (0=Left, 1=Center, 2=Right, 3=Justify). Corresponds to WdParagraphAlignment enum.",
    ),
});

async function setAlignmentTool(
  args: z.infer<typeof setAlignmentSchema>,
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set paragraph alignment to ${alignmentMap[args.alignment]}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to set paragraph alignment: ${error.message}`,
        },
      ],
      isError: true,
    };
...
async function setLeftIndentTool(
  args: z.infer<typeof setLeftIndentSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set left indent to ${args.indentPoints} points.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to set left indent: ${error.message}` },
      ],
      isError: true,
    };
  }
...
async function setRightIndentTool(
  args: z.infer<typeof setRightIndentSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set right indent to ${args.indentPoints} points.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to set right indent: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const setFirstLineIndentSchema = z.object({
  indentPoints: z
    .number()
    .describe(
      "First line indentation in points (positive for indent, negative for hanging indent).",
    ),
});

async function setFirstLineIndentTool(
  args: z.infer<typeof setFirstLineIndentSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set first line ${indentType} to ${Math.abs(args.indentPoints)} points.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to set first line indent: ${error.message}`,
        },
      ],
      isError: true,
    };
...
async function setSpaceBeforeTool(
  args: z.infer<typeof setSpaceBeforeSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set space before paragraph to ${args.spacePoints} points.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to set space before paragraph: ${error.message}`,
        },
      ],
      isError: true,
    };
...
async function setSpaceAfterTool(
  args: z.infer<typeof setSpaceAfterSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set space after paragraph to ${args.spacePoints} points.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to set space after paragraph: ${error.message}`,
        },
      ],
      isError: true,
    };
...
const setLineSpacingSchema = z.object({
  lineSpacingRule: z
    ...
    .max(5)
    .describe(
      "Line spacing rule (0=Single, 1=1.5, 2=Double, 3=AtLeast, 4=Exactly, 5=Multiple). Corresponds to WdLineSpacing enum.",
    ),
  lineSpacingValue: z
    .number()
    .optional()
    .describe(
      "Required value (in points or multiplier) if rule is AtLeast(3), Exactly(4), or Multiple(5).",
    ),
});

async function setLineSpacingTool(
  args: z.infer<typeof setLineSpacingSchema>,
): Promise<CallToolResult> {
  try {
    if (args.lineSpacingRule >= 3 && args.lineSpacingValue === undefined) {
      throw new Error(
        "lineSpacingValue is required when lineSpacingRule is AtLeast, Exactly, or Multiple.",
      );
    }
    await wordService.setParagraphLineSpacing(
      args.lineSpacingRule,
      args.lineSpacingValue,
    );
    ...
    return {
      content: [
        { type: "text", text: `Failed to set line spacing: ${error.message}` },
      ],
      isError: true,
    };
  }
