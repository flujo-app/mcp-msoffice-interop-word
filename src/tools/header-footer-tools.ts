...
const setHeaderFooterTextSchema = z.object({
  text: z
    .string()
    .max(262144)
    .describe("The text content to set in the header or footer."),
  isHeader: z
    .boolean()
    .describe("True to modify the header, False to modify the footer."),
  sectionIndex: z
    .number()
    .int()
    .min(1)
    .optional()
    .default(1)
    .describe("The 1-based index of the document section (default is 1)."),
  headerFooterType: z
    .number()
    .int()
    .min(1)
    ...
    .default(1)
    .describe(
      "Type of header/footer (1=Primary, 2=First Page, 3=Even Pages). Default is 1 (Primary). Corresponds to WdHeaderFooterIndex enum.",
    ),
});

async function setHeaderFooterTextTool(
  args: z.infer<typeof setHeaderFooterTextSchema>,
): Promise<CallToolResult> {
  try {
    await wordService.setHeaderFooterText(
      args.sectionIndex,
      args.headerFooterType,
      args.isHeader,
      args.text,
    );
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully set text for ${typeName} ${location} in section ${args.sectionIndex}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to set header/footer text: ${error.message}`,
        },
      ],
      isError: true,
    };
