...
const insertTextSchema = z.object({
  text: z
    ...
    .max(262144)
    .describe(
      "The text to insert at the current cursor position or over the selection.",
    ),
});

async function insertTextTool(
  args: z.infer<typeof insertTextSchema>,
  ...
  try {
    ...
    return {
      content: [
        { type: "text", text: `Failed to insert text: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const deleteTextSchema = z.object({
  count: z
    ...
    .default(1)
    .describe(
      "Number of units to delete. Positive deletes forward/after selection, negative deletes backward/before selection. Default is 1.",
    ),
  unit: z
    .number()
    .int()
    ...
    .default(1)
    .describe(
      "Unit to delete (1=Character, 2=Word, 3=Sentence, 4=Paragraph). Default is 1 (Character).",
    ),
});

async function deleteTextTool(
  args: z.infer<typeof deleteTextSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully deleted ${Math.abs(args.count ?? 1)} ${unitName} ${direction}.`,
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to delete text: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const findReplaceSchema = z.object({
  findText: z.string().max(262144).describe("The text to search for."),
  replaceText: z
    .string()
    .max(262144)
    .describe("The text to replace occurrences with."),
  matchCase: z
    .boolean()
    .optional()
    .default(false)
    .describe("Perform a case-sensitive search."),
  matchWholeWord: z
    .boolean()
    .optional()
    .default(false)
    .describe("Only find whole word matches."),
  replaceAll: z
    .boolean()
    .optional()
    .default(true)
    .describe("Replace all occurrences (true) or only the first one (false)."),
});

async function findReplaceTool(
  args: z.infer<typeof findReplaceSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Failed to find and replace text: ${error.message}`,
        },
      ],
      isError: true,
    };
...
async function toggleBoldTool(): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        { type: "text", text: "Toggled bold formatting for the selection." },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to toggle bold: ${error.message}` },
      ],
      isError: true,
    };
  }
...
async function toggleItalicTool(): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        { type: "text", text: "Toggled italic formatting for the selection." },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to toggle italic: ${error.message}` },
      ],
      isError: true,
    };
  }
...
const toggleUnderlineSchema = z.object({
  underlineStyle: z
    ...
    .default(1)
    .describe(
      "Optional: Underline style (WdUnderline enum value, e.g., 1=Single, 4=Double). Default is 1.",
    ),
});

async function toggleUnderlineTool(
  args: z.infer<typeof toggleUnderlineSchema>,
): Promise<CallToolResult> {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: "Toggled underline formatting for the selection.",
        },
      ],
    };
  } catch (error: any) {
    ...
    return {
      content: [
        { type: "text", text: `Failed to toggle underline: ${error.message}` },
      ],
      isError: true,
    };
  }
