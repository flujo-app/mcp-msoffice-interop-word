...
import { confineRead, confineWrite } from "./paths.js";
...
export class WordService {
  constructor(private readonly applicationFactory?: () => WordApplication) {}
   ...
   */
  public getWordApplication(): WordApplication {
    if (this.wordApp) {
      try {
        ...
        throw new Error(
          "The owned Word session is unavailable. No replacement instance was created; resolve the Word dialog or restart the server.",
        );
      }
    }

    try {
      console.error("Attempting to get or create Word.Application instance...");
      // Try to get an existing instance first, then create if not found
      // Use the destructured WinaxObject constructor
      this.wordApp = this.applicationFactory
        ? this.applicationFactory()
        : (this.createOwnedApplication() as WordApplication);
      this.wordApp.Visible = true;
      this.wordApp.DisplayAlerts = 0;
      this.wordApp.AutomationSecurity = 3;
      this.wordApp.Options.UpdateLinksAtOpen = false;
      console.error("Word.Application instance obtained successfully.");
      return this.wordApp;
    } catch (error) {
      console.error(
        "Failed to get or create Word.Application instance:",
        error,
      );
      throw new Error(
        `Failed to initialize Word.Application. Make sure Microsoft Word is installed. Error: ${error}`,
      );
    }
  }
   ...
   */
  public getActiveDocument(): WordDocument {
    const app = this.getWordApplication();
    try {
  ...
  public closeActiveDocument(saveChanges: number = 0): void {
    if (![0, -1].includes(saveChanges))
      throw new Error("Interactive save prompts are unsupported; use 0 or -1.");
    this.closeDocument(this.getActiveDocument(), saveChanges);
  }
   ...
   */
  public openDocument(filePath: string): WordDocument {
    const resolved = confineRead(filePath);
    const app = this.getWordApplication();
    try {
      // Ensure the path is absolute and correctly formatted if needed
      const openedDoc = app.Documents.Open(resolved, false, false, false);
      openedDoc.Activate();
   ...
   */
  public saveActiveDocument(): void {
    const doc = this.getActiveDocument();
    try {
      if (!doc.Path)
        throw new Error(
          "The new document has no path; use word_saveActiveDocumentAs first.",
        );
      confineRead(doc.FullName);
      doc.Save();
    } catch (error) {
   ...
   */
  public saveActiveDocumentAs(filePath: string, fileFormat?: any): void {
    const doc = this.getActiveDocument();
    try {
      ...
      doc.SaveAs2(confineWrite(filePath), format);
    } catch (error) {
      console.error(`Failed to save document as: ${filePath}`, error);
      throw new Error(
        `Failed to save document as: ${filePath}. Error: ${error}`,
      );
    }
  }
   ...
   */
  public closeDocument(doc: WordDocument, saveChanges?: any): void {
    try {
      ...
      throw new Error(`Could not close document. Error: ${error}`);
    }
  }
  ...
  public quitWord(): void {
    if (this.wordApp) {
      try {
        ...
        console.error("Word application quit successfully.");
      } catch (error) {
      ...
      console.error("Word application instance not found, nothing to quit.");
    }
  }
  ...
  public deleteText(
    ...
    try {
      ...
      if (count > 0) {
        app.Selection.Delete(unit, count);
      } else if (count < 0) {
        // Move start back and then delete forward
        app.Selection.MoveStart(unit, count); // Move start back
        app.Selection.Delete(unit, Math.abs(count)); // Delete forward
      }
  ...
  public toggleUnderline(
    underlineStyle: number = 1 /* wdUnderlineSingle */,
  ): void {
    const app = this.getWordApplication();
    try {
      const font = app.Selection.Font;
   ...
   */
  public setParagraphLeftIndent(indentPoints: number): void {
    const app = this.getWordApplication();
    try {
      app.Selection.ParagraphFormat.LeftIndent = indentPoints;
    } catch (error) {
   ...
   * @param indentPoints Indentation value in points.
   */
  public setParagraphRightIndent(indentPoints: number): void {
    const app = this.getWordApplication();
    try {
      app.Selection.ParagraphFormat.RightIndent = indentPoints;
    } catch (error) {
   ...
   */
  public setParagraphFirstLineIndent(indentPoints: number): void {
    const app = this.getWordApplication();
    try {
      app.Selection.ParagraphFormat.FirstLineIndent = indentPoints;
    } catch (error) {
   ...
   */
  public setParagraphSpaceBefore(spacePoints: number): void {
    const app = this.getWordApplication();
    try {
      app.Selection.ParagraphFormat.SpaceBefore = spacePoints;
    } catch (error) {
   ...
   * @param spacePoints Space value in points.
   */
  public setParagraphSpaceAfter(spacePoints: number): void {
    const app = this.getWordApplication();
    try {
      app.Selection.ParagraphFormat.SpaceAfter = spacePoints;
    } catch (error) {
   ...
   */
  public setParagraphLineSpacing(
    ...
    try {
      ...
      const paraFormat = app.Selection.ParagraphFormat;
      paraFormat.LineSpacingRule = lineSpacingRule;
      if (lineSpacingValue !== undefined && lineSpacingRule >= 3) {
        // Only set LineSpacing if rule requires it
  ...
  public addTable(
    ...
    try {
      ...
      const table = selection.Tables.Add(
        selection.Range,
        numRows,
        numCols,
  ...
  public getTableCell(
    ...
    try {
      ...
      console.error(
        `Failed to get cell (${rowIndex}, ${colIndex}) in table ${tableIndex}:`,
        error,
      );
  ...
  public setTableCellText(
    ...
    try {
      const cell = this.getTableCell(tableIndex, rowIndex, colIndex);
      cell.Range.Text = text;
    } catch (error) {
      console.error(
        `Failed to set text in cell (${rowIndex}, ${colIndex}) of table ${tableIndex}:`,
        error,
      );
  ...
  public insertTableRow(
    ...
    try {
      ...
      if (beforeRowIndex !== undefined) {
        if (beforeRowIndex <= 0 || beforeRowIndex > table.Rows.Count + 1) {
          // Allow inserting after last row
          throw new Error(
            `Row index ${beforeRowIndex} is out of bounds for insertion.`,
          );
        }
        const refRow =
          beforeRowIndex <= table.Rows.Count
            ? table.Rows.Item(beforeRowIndex)
            : undefined;
  ...
  public insertTableColumn(
    ...
    try {
      ...
      if (beforeColIndex !== undefined) {
        if (beforeColIndex <= 0 || beforeColIndex > table.Columns.Count + 1) {
          // Allow inserting after last col
          throw new Error(
            `Column index ${beforeColIndex} is out of bounds for insertion.`,
          );
        }
        const refCol =
          beforeColIndex <= table.Columns.Count
            ? table.Columns.Item(beforeColIndex)
            : undefined;
  ...
  public applyTableAutoFormat(
    ...
    try {
      ...
      if (typeof formatName === "string") {
        // Named Word table styles belong to Style; AutoFormat accepts a numeric WdTableFormat.
        table.Style = formatName;
      } else if (applyFormatting === undefined) table.AutoFormat(formatName);
      else
        table.AutoFormat(
          formatName,
          !!(applyFormatting & 1),
      ...
      console.error(
        `Failed to apply auto format to table ${tableIndex}:`,
        error,
      );
  ...
  public insertPicture(
    filePath: string,
    linkToFile: boolean = false,
    saveWithDocument: boolean = true,
  ): any /* Word.InlineShape */ {
    const app = this.getWordApplication();
    try {
      const selection = app.Selection;
      const inlineShape = selection.InlineShapes.AddPicture(
        confineRead(filePath),
        linkToFile,
        saveWithDocument,
        selection.Range,
      );
  ...
  public setInlinePictureSize(
    ...
    try {
      ...
      console.error(
        `Failed to set size for inline shape ${shapeIndex}:`,
        error,
      );
  ...
  public getHeaderFooter(
    ...
    try {
      ...
      if (headerFooterType < 1 || headerFooterType > 3) {
        throw new Error(
          `Invalid header/footer type: ${headerFooterType}. Use 1, 2, or 3.`,
        );
      }
      ...
      if (!headerFooter?.Exists) {
        ...
        // For now, let's throw.
        throw new Error(
          `The requested ${isHeader ? "header" : "footer"} type (${headerFooterType}) does not exist or is not active for section ${sectionIndex}. Check document settings.`,
        );
      }
      return headerFooter;
    } catch (error) {
      console.error(
        `Failed to get ${isHeader ? "header" : "footer"} type ${headerFooterType} for section ${sectionIndex}:`,
        error,
      );
  ...
  public setHeaderFooterText(
    ...
    try {
      const headerFooter = this.getHeaderFooter(
        sectionIndex,
        headerFooterType,
        isHeader,
      );
      ...
      console.error(
        `Failed to set text for ${isHeader ? "header" : "footer"} type ${headerFooterType} section ${sectionIndex}:`,
        error,
      );
   ...
   */
  public setPageOrientation(orientation: number): void {
    const doc = this.getActiveDocument();
    try {
   ...
   */
  public selectParagraph(paragraphIndex: number): void {
    const doc = this.getActiveDocument();
    try {
   ...
   */
  public collapseSelection(toStart: boolean = true): void {
    const app = this.getWordApplication();
    try {
      const selection = app.Selection;
