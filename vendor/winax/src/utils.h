...
// Detect the optional member in a substitution context: Node 22 has This(),
// while newer V8 versions expose HolderV2(). A missing member is not an error.
template<class T, class = void>
struct is_holder_v2 : std::false_type {};
template<class T>
struct is_holder_v2<T, std::void_t<decltype(std::declval<const T&>().HolderV2())>> : std::true_type {};

template<class T>
inline typename std::enable_if<!is_holder_v2<T>::value, Local<Object>>::type
